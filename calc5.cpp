    ...
    struct error_handler_
    {
          ...
          , Iterator err_pos, Iterator last) const
        {
            std::cout
                ...
                << what                         // what failed?
                << " here: \""
            //   << xs(err_pos, last)   // iterators to error-pos, end
                << "\""
                << std::endl
            ;
        }
...
main()
{
    ...
    typedef xs::const_iterator iterator_type;
    ...
    while (std::getline(std::cin, stra))
    {
        if (stra.empty() || stra[0] == 'q' || stra[0] == 'Q')
            break;
	...
	xs str = *((xs*)(&stra));	
        xs::const_iterator iter = str.begin();
	xs::const_iterator end = str.end();
